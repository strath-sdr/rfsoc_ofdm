...
ARCHITECTURE rtl OF ofdm_rx_src_OFDMRx IS
  ...
  SIGNAL reduced_reg                      : std_logic_vector(0 TO 767);  -- ufix1 [768]
  ...
  SIGNAL enable_threshold_1               : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Bit_Slice2_out1                  : std_logic;  -- ufix1
  SIGNAL Constant_out1                    : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL threshold_1                      : unsigned(31 DOWNTO 0);  -- uint32
  ...
  SIGNAL Repeat2_out1                     : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL delayMatch1_reg                  : vector_of_unsigned32(0 TO 767);  -- ufix32 [768]
  SIGNAL Repeat2_out1_1                   : unsigned(31 DOWNTO 0);  -- uint32
  ...
  SIGNAL Upsample2_int_delay_reg          : std_logic_vector(0 TO 4);  -- ufix1 [5]
  SIGNAL Upsample2_out1                   : std_logic;
  SIGNAL delayMatch3_reg                  : std_logic_vector(0 TO 767);  -- ufix1 [768]
  SIGNAL Upsample2_out1_1                 : std_logic;
...
BEGIN
  ...
  Bit_Slice3_out1 <= select_unsigned(0);
  ...
  BEGIN
    ...
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb = '1' THEN
        reduced_reg(0) <= Bit_Slice3_out1;
        reduced_reg(1 TO 767) <= reduced_reg(0 TO 766);
      END IF;
    END IF;
  END PROCESS reduced_process;

  Bit_Slice3_out1_1 <= reduced_reg(767);

  data_unsigned <= unsigned(data);

  Bit_Slice_out1 <= data_unsigned(15 DOWNTO 0);

  Bit_Slice1_out1 <= data_unsigned(31 DOWNTO 16);

  Data_Type_Conversion_out1_re <= signed(Bit_Slice_out1);
  Data_Type_Conversion_out1_im <= signed(Bit_Slice1_out1);

  enable_threshold_1 <= unsigned(enable_threshold);

  Bit_Slice2_out1 <= enable_threshold_1(0);

  Constant_out1 <= to_unsigned(1073741824, 32);

  threshold_1 <= unsigned(threshold);
  ...
  BEGIN
    ...
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb = '1' THEN
        delayMatch1_reg(0) <= Repeat2_out1;
        delayMatch1_reg(1 TO 767) <= delayMatch1_reg(0 TO 766);
      END IF;
    END IF;
  END PROCESS delayMatch1_process;

  Repeat2_out1_1 <= delayMatch1_reg(767);

  
  Multiport_Switch1_out1 <= Bit_Concat_out1 WHEN Bit_Slice3_out1_1 = '0' ELSE
      Repeat2_out1_1;

  m_axis_tdata <= std_logic_vector(Multiport_Switch1_out1);

  -- Upsample: Upsample by 256, Sample offset 5 
  Upsample_zero <= '0';

  
  Upsample_muxout <= PhaseTracking_2_out2 WHEN enb_1_256_1 = '1' ELSE
      Upsample_zero;
  ...
  -- Upsample2: Upsample by 256, Sample offset 5 
  Upsample2_zero <= '0';

  
  Upsample2_muxout <= Constant1_out1 WHEN enb_1_256_1 = '1' ELSE
      Upsample2_zero;
  ...
  -- Upsample sample offset register
  Upsample2_int_delay_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      Upsample2_int_delay_reg <= (OTHERS => '0');
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb = '1' AND enb = '1' THEN
        Upsample2_int_delay_reg(0) <= Upsample2_bypassout;
        Upsample2_int_delay_reg(1 TO 4) <= Upsample2_int_delay_reg(0 TO 3);
      END IF;
    END IF;
  END PROCESS Upsample2_int_delay_process;

  Upsample2_out1 <= Upsample2_int_delay_reg(4);
  ...
  BEGIN
    ...
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb = '1' THEN
        delayMatch3_reg(0) <= Upsample2_out1;
        delayMatch3_reg(1 TO 767) <= delayMatch3_reg(0 TO 766);
      END IF;
    END IF;
  END PROCESS delayMatch3_process;

  Upsample2_out1_1 <= delayMatch3_reg(767);

  
  Multiport_Switch2_out1 <= Upsample_out1 WHEN Bit_Slice3_out1_1 = '0' ELSE
      Upsample2_out1_1;

  ce_out <= enb_1_1_1;

  m_axis_tvalid <= Multiport_Switch2_out1;
