-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj\OFDM_rx\hdlsrc\OFDM_Tx_Rx_HW\ofdm_rx_src_Gradient.vhd
...
-- Source Path: OFDM_Tx_Rx_HW/OFDMRx/PhaseTracking_1/GradientEstimate /Gradient
-- Hierarchy Level: 3
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
...
ARCHITECTURE rtl OF ofdm_rx_src_Gradient IS
  ...
  SIGNAL Delay2_out1                      : signed(22 DOWNTO 0);  -- sfix23_En20
...
BEGIN
  ...
  BEGIN
    IF reset = '1' THEN
      Delay2_out1 <= to_signed(16#000000#, 23);
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        Delay2_out1 <= diffIn_signed;
      END IF;
    END IF;
  ...
  Add_sub_cast <= resize(Delay1_out1, 24);
  Add_sub_cast_1 <= resize(Delay2_out1, 24);
  ...
  diffOut <= std_logic_vector(Gain_out1_1);
