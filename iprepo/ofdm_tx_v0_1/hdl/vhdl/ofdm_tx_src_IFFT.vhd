-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj\OFDM_tx\hdlsrc\OFDM_Tx_Rx_HW\ofdm_tx_src_IFFT.vhd
-- Created: 2021-01-08 12:14:22
-- 
-- Generated by MATLAB 9.8 and HDL Coder 3.16
-- 
-- -------------------------------------------------------------


-- -------------------------------------------------------------
-- 
-- Module: ofdm_tx_src_IFFT
-- Source Path: OFDM_Tx_Rx_HW/OFDMTx/IFFT
-- Hierarchy Level: 1
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.ALL;
USE IEEE.numeric_std.ALL;

ENTITY ofdm_tx_src_IFFT IS
  PORT( clk                               :   IN    std_logic;
        reset                             :   IN    std_logic;
        enb_1_192_0                       :   IN    std_logic;
        dataIn_re                         :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        dataIn_im                         :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        preambleValidIn                   :   IN    std_logic;
        dataValidIn                       :   IN    std_logic;
        dataOut_re                        :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        dataOut_im                        :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        preambValidOut                    :   OUT   std_logic;
        dataValidOut                      :   OUT   std_logic
        );
END ofdm_tx_src_IFFT;


ARCHITECTURE rtl OF ofdm_tx_src_IFFT IS

  -- Component Declarations
  COMPONENT ofdm_tx_src_IFFTValid
    PORT( clk                             :   IN    std_logic;
          reset                           :   IN    std_logic;
          enb_1_192_0                     :   IN    std_logic;
          preambleValid                   :   IN    std_logic;
          dataValid                       :   IN    std_logic;
          ifftValid                       :   OUT   std_logic
          );
  END COMPONENT;

  COMPONENT ofdm_tx_src_IFFT_HDL_Optimized
    PORT( clk                             :   IN    std_logic;
          reset                           :   IN    std_logic;
          enb_1_192_0                     :   IN    std_logic;
          dataIn_re                       :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          dataIn_im                       :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          validIn                         :   IN    std_logic;
          dataOut_re                      :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          dataOut_im                      :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          validOut                        :   OUT   std_logic
          );
  END COMPONENT;

  -- Component Configuration Statements
  FOR ALL : ofdm_tx_src_IFFTValid
    USE ENTITY work.ofdm_tx_src_IFFTValid(rtl);

  FOR ALL : ofdm_tx_src_IFFT_HDL_Optimized
    USE ENTITY work.ofdm_tx_src_IFFT_HDL_Optimized(rtl);

  -- Signals
  SIGNAL dataIn_re_signed                 : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL dataIn_im_signed                 : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL dataIn_re_1                      : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL dataIn_im_1                      : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL preambleValidIn_1                : std_logic;
  SIGNAL dataValidIn_1                    : std_logic;
  SIGNAL IFFTValid_out1                   : std_logic;
  SIGNAL IFFTValid_out1_1                 : std_logic;
  SIGNAL IFFT_HDL_Optimized_out1_re       : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL IFFT_HDL_Optimized_out1_im       : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL IFFT_HDL_Optimized_out2          : std_logic;
  SIGNAL IFFT_HDL_Optimized_out1_re_signed : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL IFFT_HDL_Optimized_out1_im_signed : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL IFFT_HDL_Optimized_out1_re_1     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL IFFT_HDL_Optimized_out1_im_1     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay_reg                        : std_logic_vector(0 TO 172);  -- ufix1 [173]
  SIGNAL Delay_out1                       : std_logic;
  SIGNAL Delay_out1_1                     : std_logic;
  SIGNAL Delay1_reg                       : std_logic_vector(0 TO 172);  -- ufix1 [173]
  SIGNAL Delay1_out1                      : std_logic;
  SIGNAL Delay1_out1_1                    : std_logic;

BEGIN
  u_IFFTValid : ofdm_tx_src_IFFTValid
    PORT MAP( clk => clk,
              reset => reset,
              enb_1_192_0 => enb_1_192_0,
              preambleValid => preambleValidIn_1,
              dataValid => dataValidIn_1,
              ifftValid => IFFTValid_out1
              );

  u_IFFT_HDL_Optimized : ofdm_tx_src_IFFT_HDL_Optimized
    PORT MAP( clk => clk,
              reset => reset,
              enb_1_192_0 => enb_1_192_0,
              dataIn_re => std_logic_vector(dataIn_re_1),  -- sfix16_En14
              dataIn_im => std_logic_vector(dataIn_im_1),  -- sfix16_En14
              validIn => IFFTValid_out1_1,
              dataOut_re => IFFT_HDL_Optimized_out1_re,  -- sfix16_En14
              dataOut_im => IFFT_HDL_Optimized_out1_im,  -- sfix16_En14
              validOut => IFFT_HDL_Optimized_out2
              );

  dataIn_re_signed <= signed(dataIn_re);

  dataIn_im_signed <= signed(dataIn_im);

  in_0_pipe_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      dataIn_re_1 <= to_signed(16#0000#, 16);
      dataIn_im_1 <= to_signed(16#0000#, 16);
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        dataIn_re_1 <= dataIn_re_signed;
        dataIn_im_1 <= dataIn_im_signed;
      END IF;
    END IF;
  END PROCESS in_0_pipe_process;


  in_1_pipe_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      preambleValidIn_1 <= '0';
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        preambleValidIn_1 <= preambleValidIn;
      END IF;
    END IF;
  END PROCESS in_1_pipe_process;


  in_2_pipe_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      dataValidIn_1 <= '0';
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        dataValidIn_1 <= dataValidIn;
      END IF;
    END IF;
  END PROCESS in_2_pipe_process;


  delayMatch_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      IFFTValid_out1_1 <= '0';
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        IFFTValid_out1_1 <= IFFTValid_out1;
      END IF;
    END IF;
  END PROCESS delayMatch_process;


  IFFT_HDL_Optimized_out1_re_signed <= signed(IFFT_HDL_Optimized_out1_re);

  IFFT_HDL_Optimized_out1_im_signed <= signed(IFFT_HDL_Optimized_out1_im);

  out_0_pipe_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      IFFT_HDL_Optimized_out1_re_1 <= to_signed(16#0000#, 16);
      IFFT_HDL_Optimized_out1_im_1 <= to_signed(16#0000#, 16);
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        IFFT_HDL_Optimized_out1_re_1 <= IFFT_HDL_Optimized_out1_re_signed;
        IFFT_HDL_Optimized_out1_im_1 <= IFFT_HDL_Optimized_out1_im_signed;
      END IF;
    END IF;
  END PROCESS out_0_pipe_process;


  dataOut_re <= std_logic_vector(IFFT_HDL_Optimized_out1_re_1);

  dataOut_im <= std_logic_vector(IFFT_HDL_Optimized_out1_im_1);

  Delay_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      Delay_reg <= (OTHERS => '0');
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        Delay_reg(0) <= preambleValidIn_1;
        Delay_reg(1 TO 172) <= Delay_reg(0 TO 171);
      END IF;
    END IF;
  END PROCESS Delay_process;

  Delay_out1 <= Delay_reg(172);

  out_1_pipe_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      Delay_out1_1 <= '0';
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        Delay_out1_1 <= Delay_out1;
      END IF;
    END IF;
  END PROCESS out_1_pipe_process;


  Delay1_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      Delay1_reg <= (OTHERS => '0');
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        Delay1_reg(0) <= dataValidIn_1;
        Delay1_reg(1 TO 172) <= Delay1_reg(0 TO 171);
      END IF;
    END IF;
  END PROCESS Delay1_process;

  Delay1_out1 <= Delay1_reg(172);

  out_2_pipe_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      Delay1_out1_1 <= '0';
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb_1_192_0 = '1' THEN
        Delay1_out1_1 <= Delay1_out1;
      END IF;
    END IF;
  END PROCESS out_2_pipe_process;


  preambValidOut <= Delay_out1_1;

  dataValidOut <= Delay1_out1_1;

END rtl;

